...
- **Поддержка 16 расширителей**: До 16 PCF8574(A) на одной шине I2C (адреса 0x20-0x27 и 0x38-0x3F)
- **Режимы работы GPIO**: Поддержка выходных и входных режимов для каждого из 8 GPIO
- **Генерация прерываний**: Поддержка прерываний от входных GPIO (отрицательный фронт)
- **Статистика ошибок**: Встроенные счетчики ошибок I2C, событий, векторов и переполнения очереди
- **Потокобезопасность**: Использует драйвер I2C ESP-IDF и FreeRTOS
- **Минимальные накладные расходы**: Низкие требования к памяти и процессору
- **Простой API**: Легкое чтение/запись состояния всех GPIO и отдельных пинов
...
typedef struct
{
    ...
    uint32_t stack_size;                // Размер стека задачи обработки прерываний
...
Непрозрачный дескриптор экземпляра устройства PCF8574. Внутренняя структура минимальна — хранит только I2C-адрес; полное состояние устройства поддерживается во внутреннем векторе.
...
- `config` - Указатель на структуру конфигурации инициализации PCF8574. Не должен быть NULL
- `handle` - Указатель на указатель на дескриптор PCF8574. Должен быть NULL
...
- `handle` - Указатель на указатель на дескриптор PCF8574. Не должен быть NULL
...
- `handle` - Указатель на указатель на дескриптор PCF8574. Не должен быть NULL
- `reg` - Указатель для хранения состояния GPIO (биты 0-7). Не должен быть NULL
...
- `status` - Указатель для хранения состояния GPIO (true - HIGH, false - LOW). Не должен быть NULL
...
"*Сгенерировано для zh_pcf8574 v3.0.0*"
